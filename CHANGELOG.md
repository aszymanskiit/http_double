...
- MockServer `PUT /mockserver/clear` (and `/clear`) with an `httpRequest` matcher, or with
  top-level `path` / `pathRegex` / `method`, now removes only matching expectation rules.
  An empty body still clears all rules (previous behaviour).
